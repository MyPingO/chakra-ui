...
Chakra factory serves as a way to create supercharged JSX components from any
HTML element to enable them to receive JSX style props.
...
With the JSX factory, you can use the `chakra.<element>` syntax to create JSX
...
Here's an example that forwards all props that don't start with `$`
